...
public partial class PoolTurnResolver : TurnResolver
{
    ...
    internal const int SnapshotRequestsPerSecond = 2;
    internal const int PushOutRequestsPerSecond = 6;
    internal const int MaximumTrackedRequestPeers = 16;
    ...
    private readonly PeerRequestRateLimiter _snapshotRequestLimiter = new(
        SnapshotRequestsPerSecond, 1000, MaximumTrackedRequestPeers);
    private readonly PeerRequestRateLimiter _pushOutRequestLimiter = new(
        PushOutRequestsPerSecond, 1000, MaximumTrackedRequestPeers);
    ...
    public override void Setup(TableGame tableGame)
    {
        ...
        _snapshotRequestLimiter.Clear();
        _pushOutRequestLimiter.Clear();
        ResetPushOutState();
    ...
    private void RequestFullStateOnServer()
    {
        ...
        if (!TryConsumeSnapshotRequest(requesterId))
            return;

        SendFullState(requesterId);
    }

    private bool TryConsumeSnapshotRequest(int requesterId) =>
        _snapshotRequestLimiter.TryConsume(requesterId);

    internal bool TryConsumeSnapshotRequest(int requesterId, ulong nowMilliseconds) =>
        _snapshotRequestLimiter.TryConsume(requesterId, nowMilliseconds);
    ...
    private void DeclarePushOutOnServer()
    {
        ...
        var requesterId = Multiplayer.GetRemoteSenderId();
        if (!TryConsumePushOutRequest(requesterId))
            return;

        TryDeclarePushOut(requesterId);
    }
    ...
    private void ResolvePushOutChoiceOnServer(bool passBack)
    {
        ...
        TryResolvePushOutChoice(requesterId, passBack);
    }

    private bool TryConsumePushOutRequest(int requesterId) =>
        _pushOutRequestLimiter.TryConsume(requesterId);

    internal bool TryConsumePushOutRequest(int requesterId, ulong nowMilliseconds) =>
        _pushOutRequestLimiter.TryConsume(requesterId, nowMilliseconds);
