...
public partial class SceneLoadTest : Node
{
    ...
    public override void _Ready()
    {
        ...
        if (ballScene != null)
        {
            TestBallReplicationIsSpawnOnly(ballScene);
        ...
        TestAuthorityStateSynchronizerProtocol();
        TestTableStartAndStateRequestGuards();
        TestSurrenderRequestGuard();
        TestPoolRequestRateLimiters();
        TestBallPlacementPreviewChannel();
        TestPoolPublicSnapshotRoundTrip();
        TestCountdownDropsMissingPlayers();
        TestPoolPlayerCapacity();
    ...
    private void TestBallReplicationIsSpawnOnly(PackedScene ballScene)
    {
        ...
        var synchronizer = ball.GetNodeOrNull<MultiplayerSynchronizer>("MultiplayerSynchronizer");
        var config = synchronizer?.ReplicationConfig;
        var positionPath = new NodePath(".:position");

        Check("posição da bola faz parte do snapshot de spawn",
            config != null && config.PropertyGetSpawn(positionPath));
        Check("posição da bola não cria um stream confiável durante a tacada",
            config != null && (int)config.PropertyGetReplicationMode(positionPath) == 0);

        ball.Free();
    }

    private void TestAuthorityStateSynchronizerProtocol()
    {
        var request = typeof(AuthorityStateSynchronizer).GetMethod(
            "RequestStateChangeOnServer", BindingFlags.Instance | BindingFlags.NonPublic);
        var apply = typeof(AuthorityStateSynchronizer).GetMethod(
            "ApplyStateFromServer", BindingFlags.Instance | BindingFlags.NonPublic);
        var requestRpc = request?.GetCustomAttribute<RpcAttribute>();
        var applyRpc = apply?.GetCustomAttribute<RpcAttribute>();

        Check("estado do cliente viaja por uma RPC de request AnyPeer",
            requestRpc != null
            && requestRpc.Mode == MultiplayerApi.RpcMode.AnyPeer
            && !requestRpc.CallLocal
            && requestRpc.TransferMode == MultiplayerPeer.TransferModeEnum.Reliable);
        Check("estado validado volta por uma RPC Authority do servidor",
            applyRpc != null
            && applyRpc.Mode == MultiplayerApi.RpcMode.Authority
            && !applyRpc.CallLocal
            && applyRpc.TransferMode == MultiplayerPeer.TransferModeEnum.Reliable
            && AuthorityStateSynchronizer.ServerPeerId == 1);

        var machine = new StateMachine();
        var knownState = new State { Type = "KNOWN" };
        machine.States[knownState.Type] = knownState;
        var synchronizer = new AuthorityStateSynchronizer { StateMachine = machine };

        var validMetadata = new Dictionary
        {
            ["animation"] = "Strike",
            ["players_ids"] = Variant.From(new[] { "1", "2" }),
        };
        Check("payload conhecido e limitado é aceito",
            synchronizer.IsStatePayloadValid(knownState.Type, validMetadata));
        Check("tipo de estado desconhecido é rejeitado",
            !synchronizer.IsStatePayloadValid("UNKNOWN", validMetadata));
        Check("metadado com objeto Godot é rejeitado",
            !AuthorityStateSynchronizer.IsMetadataValid(
                new Dictionary { ["node"] = this }));
        Check("string de metadado acima do limite é rejeitada",
            !AuthorityStateSynchronizer.IsMetadataValid(
                new Dictionary
                {
                    ["animation"] = new string('x',
                        AuthorityStateSynchronizer.MaxMetadataStringLength + 1),
                }));

        var acceptedRequests = true;
        for (var requestIndex = 0;
            requestIndex < AuthorityStateSynchronizer.StateRequestsPerSecond;
            requestIndex++)
        {
            acceptedRequests &= synchronizer.TryConsumeStateRequest(55, 500);
        }

        Check("requests de estado têm orçamento antes de validar ou retransmitir",
            acceptedRequests && !synchronizer.TryConsumeStateRequest(55, 500));
    ...
    private void TestTableStartAndStateRequestGuards()
    {
        var table = new Table();
        var game = new TableGame();
        var machine = new StateMachine();
        var waiting = new WaitingGameStart
        {
            Parent = table,
            StateMachine = machine,
        };
        var started = new State { Type = StatesRef.GameStarted };
        table.CurrentTableGame = game;
        machine.Current = waiting;

        Check("pedido de início só é aceito enquanto Waiting é o estado corrente",
            waiting.CanAcceptStartRequest());
        machine.Current = started;
        Check("nó Waiting inativo não pode reiniciar a máquina de estados",
            !waiting.CanAcceptStartRequest());

        machine.Current = waiting;
        game.PrepareMatch(new Array(), "");
        Check("partida ativa não pode ser reinicializada por outro request",
            !waiting.CanAcceptStartRequest());

        var acceptedRequests = true;
        for (var requestIndex = 0;
            requestIndex < WaitingGameStart.StartRequestsPerSecond;
            requestIndex++)
        {
            acceptedRequests &= waiting.TryConsumeStartRequest(56, 600);
        }

        Check("pedido reliable de início de mesa tem orçamento por peer",
            acceptedRequests && !waiting.TryConsumeStartRequest(56, 600));
    ...
    private void TestPoolRequestRateLimiters()
    {
        var resolver = new PoolTurnResolver();
        var snapshotWindowAccepted = true;
        for (var request = 0; request < PoolTurnResolver.SnapshotRequestsPerSecond; request++)
            snapshotWindowAccepted &= resolver.TryConsumeSnapshotRequest(41, 100);

        Check("sinuca limita snapshots completos por peer antes de gerar a resposta",
            snapshotWindowAccepted && !resolver.TryConsumeSnapshotRequest(41, 100));

        var pushOutWindowAccepted = true;
        for (var request = 0; request < PoolTurnResolver.PushOutRequestsPerSecond; request++)
            pushOutWindowAccepted &= resolver.TryConsumePushOutRequest(41, 200);

        Check("declaração e escolha de push-out compartilham um teto por peer",
            pushOutWindowAccepted && !resolver.TryConsumePushOutRequest(41, 200));
        resolver.Free();

        var cueBridge = new CueNetworkBridge();
        var strikeWindowAccepted = true;
        for (var request = 0; request < CueNetworkBridge.StrikeRequestsPerSecond; request++)
            strikeWindowAccepted &= cueBridge.TryConsumeStrikeRequest(42, 300);

        Check("pedidos reliable de tacada têm um teto independente por peer",
            strikeWindowAccepted && !cueBridge.TryConsumeStrikeRequest(42, 300));
        cueBridge.Free();

        var placement = new BallPlacementManager();
        var placementWindowAccepted = true;
        for (var request = 0; request < BallPlacementManager.PlacementRequestsPerSecond; request++)
            placementWindowAccepted &= placement.TryConsumePlacementRequest(43, 400);

        Check("início e confirmação de ball-in-hand compartilham um teto por peer",
            placementWindowAccepted && !placement.TryConsumePlacementRequest(43, 400));
        placement.Free();
    }

    private void TestSurrenderRequestGuard()
    {
        var game = new TableGame();
        var acceptedWindow = true;
        for (var request = 0; request < TableGame.SurrenderRequestsPerSecond; request++)
            acceptedWindow &= game.TryConsumeSurrenderRequest(61, 700);

        Check("pedidos reliable de desistência têm orçamento antes de validar o ID",
            acceptedWindow && !game.TryConsumeSurrenderRequest(61, 700));
        Check("orçamento de desistência é independente por peer",
            game.TryConsumeSurrenderRequest(62, 700));
        Check("janela de desistência reabre deterministicamente",
            game.TryConsumeSurrenderRequest(61, 1_700));
        Check("peer não pode desistir em nome de outro jogador",
            !TableGame.IsSurrenderIdentityValid(61, "62"));
        Check("ID vazio ou inválido é descartado silenciosamente",
            !TableGame.IsSurrenderIdentityValid(61, null)
            && !TableGame.IsSurrenderIdentityValid(0, "0"));

        for (var peer = 2_000;
            peer < 2_000 + TableGame.MaximumTrackedSurrenderPeers * 2;
            peer++)
        {
            game.TryConsumeSurrenderRequest(peer, 1_700);
        }

        Check("tabela de rate limit de desistência permanece limitada",
            game.TrackedSurrenderPeerCount <= TableGame.MaximumTrackedSurrenderPeers);
        game.Free();
    }

    private void TestBallPlacementPreviewChannel()
    {
        var requestMove = typeof(BallPlacementManager).GetMethod(
            "RequestMovePlacementPreview", BindingFlags.Instance | BindingFlags.NonPublic);
        var updatePreview = typeof(BallPlacementManager).GetMethod(
            "UpdatePlacementPreview", BindingFlags.Instance | BindingFlags.NonPublic);
        var requestRpc = requestMove?.GetCustomAttribute<RpcAttribute>();
        var updateRpc = updatePreview?.GetCustomAttribute<RpcAttribute>();

        Check("preview cliente-servidor usa unreliable ordered no canal exclusivo 4",
            requestRpc != null
            && requestRpc.TransferMode == MultiplayerPeer.TransferModeEnum.UnreliableOrdered
            && requestRpc.TransferChannel == BallPlacementManager.PreviewTransferChannel);
        Check("preview servidor-espectadores usa unreliable ordered no canal exclusivo 4",
            updateRpc != null
            && updateRpc.TransferMode == MultiplayerPeer.TransferModeEnum.UnreliableOrdered
    ...
    private void TestRackSpacing(PackedScene ballScene)
    {
        ...
        var cueScene = GD.Load<PackedScene>("res://Games/Pool/Components/Cue/Cue.tscn");
    ...
    private void TestPoolPlayerCapacity()
    {
        var game = new PoolGame();

        Check("sinuca declara capacidade de um a dois jogadores",
            game.MinimumPlayers == 1 && game.MaximumPlayers == 2 && game.MaxPlayers == 2);
        Check("sinuca aceita os dois tamanhos de partida suportados",
            game.CanStartWith(1) && game.CanStartWith(2));
        Check("sinuca rejeita contagens fora dos limites",
            !game.CanStartWith(0) && !game.CanStartWith(3));
