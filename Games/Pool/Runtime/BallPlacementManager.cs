...
{
    internal const int PlacementRequestsPerSecond = 8;
    internal const int MaximumTrackedPlacementPeers = 16;
    internal const int PreviewTransferChannel = 4;
    ...
    private readonly PeerRequestRateLimiter _placementRequestLimiter = new(
        PlacementRequestsPerSecond, 1000, MaximumTrackedPlacementPeers);
    ...
    public void CancelPlacement()
    {
        ...
        _placementRequestLimiter.Clear();
    ...
    private void RequestStartPlacementPreview(NodePath ballPath, Vector3 position)
    {
        ...
        if (!TryConsumePlacementRequest(requesterId))
            return;

        TryStartPreview(requesterId, ballPath, position);
    }

    [Rpc(MultiplayerApi.RpcMode.AnyPeer, CallLocal = false,
        TransferMode = MultiplayerPeer.TransferModeEnum.UnreliableOrdered,
        TransferChannel = PreviewTransferChannel)]
    ...
    private void RequestConfirmPlacement(NodePath ballPath, Vector3 position)
    {
        ...
        TryConfirmPlacement(requesterId, ballPath, position);
    }

    private bool TryConsumePlacementRequest(int requesterId) =>
        _placementRequestLimiter.TryConsume(requesterId);

    internal bool TryConsumePlacementRequest(int requesterId, ulong nowMilliseconds) =>
        _placementRequestLimiter.TryConsume(requesterId, nowMilliseconds);
