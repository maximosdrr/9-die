...
public partial class DominoSceneLoadTest : Node
{
    ...
    public override async void _Ready()
    {
        ...
        // QueueFree() is used by the scene fixtures below. ProcessFrame resumes at the start of
        // a frame, so wait for the following signal as well: that leaves one complete frame for
        // Godot to drain its deletion queue before shutdown diagnostics are collected.
    ...
    {
        LoadScene("res://Games/Domino/Domino.tscn");
    ...
    {
        var scene = GD.Load<PackedScene>("res://Games/Domino/Domino.tscn");
        if (scene == null)
            return;
    ...
    private void TestActionRateLimiter()
    {
        ...
        for (var request = 0; request < DominoTurnResolver.ActionRequestsPerSecond; request++)
            acceptedWindow &= resolver.TryConsumeActionRequest(31, 500);

        Check($"dominó compartilha um teto de {DominoTurnResolver.ActionRequestsPerSecond} "
            + "pedidos entre jogar, comprar e passar", acceptedWindow);
        Check("dominó descarta o pedido excedente antes de avaliar a jogada",
            !resolver.TryConsumeActionRequest(31, 500));
        Check("um peer não consome a janela de ações de outro",
            resolver.TryConsumeActionRequest(32, 500));
        Check("a janela de ações do dominó reabre no limite exato",
            resolver.TryConsumeActionRequest(31, 1500));
    ...
    private void TestHandViewIsASeam()
    {
        ...
        Check("o controlador aponta para a mão em 3D",
            wiredHandView is DominoHand3DView);
        wiredHandView?.Free();
        wired?.Free();
    ...
    private void TestHandStateMachine()
    {
        var scene = GD.Load<PackedScene>(
            "res://Games/Domino/GameController/Views/DominoHand3DView.tscn");
        if (scene == null)
            return;
    ...
    private void TestSeatPresenter()
    {
        ...
        // NameOf() intentionally resolves through the same registry used at runtime. A detached
        // container keeps these lightweight Player doubles out of _Ready() while still providing
        // a truthful registry fixture for both occupied seats.
        var registry = PlayerRegistry.Instance;
        var previousPlayersContainer = registry?.PlayersContainer;
        var playersContainer = new Node3D();
        var fakeTurnOwner = new Player { Name = "seat-test", Nickname = "Seat Test" };
        var occupiedPlayer = new Player { Name = "occupied-test", Nickname = "Occupied Test" };
        playersContainer.AddChild(fakeTurnOwner);
        playersContainer.AddChild(occupiedPlayer);
        if (registry != null)
            registry.PlayersContainer = playersContainer;
        ...
        game.Player = null;
        if (registry != null)
            registry.PlayersContainer = previousPlayersContainer;
        playersContainer.Free();
    ...
    {
        var scene = GD.Load<PackedScene>("res://App/Main.tscn");
        if (scene == null)
        {
    ...
    {
        var scene = GD.Load<PackedScene>("res://Games/Domino/GameController/DominoController.tscn");
        if (scene == null)
            return;
    ...
    /// <summary>
    /// Scene fixtures are deliberately short-lived. Free any fixture still parented to the test
    /// immediately; QueueFree would otherwise keep large render resources alive until shutdown.
    /// </summary>
