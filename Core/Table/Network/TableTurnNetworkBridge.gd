...
func setup(_table_game: TableGame) -> void:
	...
	if not table_game.match_over.is_connected(_on_match_over_server_side):
		table_game.match_over.connect(_on_match_over_server_side)

func _on_match_started_server_side(players_ids: Array, first_player: String) -> void:
	if multiplayer.is_server():
		_rpc_sync_match_setup.rpc(players_ids, first_player)
...
func _on_match_over_server_side(winner: String, context: Dictionary):
	if multiplayer.is_server():
		_rpc_sync_match_over.rpc(winner, context)

@rpc("authority", "call_remote", "reliable")
func _rpc_sync_match_setup(players_ids: Array, first_player: String) -> void:
	table_game.setup_match(players_ids, first_player)
...
@rpc("authority", "call_remote", "reliable")
func _rpc_sync_match_over(winner: String, context: Dictionary) -> void:
	table_game.call_match_over(winner, context)
