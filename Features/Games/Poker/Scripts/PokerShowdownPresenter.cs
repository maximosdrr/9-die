...
public partial class PokerShowdownPresenter : Node3D
{
	...
	private bool TryStart(float delta, bool blocked)
	{
		...
		for (var row = 0; row < ranked.Count; row++)
		{
			...
			for (var cardIndex = 0; cardIndex < result.Cards.Length; cardIndex++)
			{
				...
				card.Visible = true;
				_moves.Add(new CardMove { Card = card, From = source, To = target,
	...
	private void BuildPool(PackedScene cardScene)
	{
		...
		for (var i = 0; i < 4; i++)
		{
			var label = new Label3D { Name = $"ShowdownLabel{i}",
				Billboard = BaseMaterial3D.BillboardModeEnum.Enabled, NoDepthTest = true,
				PixelSize = 0.00020f, FontSize = 64, OutlineSize = 10, Visible = false };
			AddChild(label);
			_labels.Add(label);
	...
	private float ReaderYaw()
	{
		...
		var reader = new Vector2(local.X, local.Z);
