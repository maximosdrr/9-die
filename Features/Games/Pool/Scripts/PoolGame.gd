...
func spawn_triangle_rack() -> void:
	...
	for row in range(rows):
		...
		for col in range(row + 1):
			if index >= COLORED_BALL_MESHES.size():
				break
				
			var mesh_scene = COLORED_BALL_MESHES[index]
